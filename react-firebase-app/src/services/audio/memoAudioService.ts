...
export interface AudioProcessingProgress {
  stage: 'uploading' | 'transcribing' | 'saving' | 'complete' | 'error';
...
class MemoAudioService {
  ...
  async createAudioMemo(
    ...
    try {
      ...
      const initialMemoData: CreateMemoData = {
        ...memoData,
        userId
      };
  ...
  async batchTranscribeMemos(
    memoIds: string[],
    options: AudioProcessingOptions = {},
    onProgress?: (memoId: string, progress: AudioProcessingProgress) => void
   ...
   * 音声ファイルサイズの最適化
   */
  async optimizeAudioBlob(audioBlob: Blob, _targetQuality: 'low' | 'medium' | 'high' = 'medium'): Promise<Blob> {
